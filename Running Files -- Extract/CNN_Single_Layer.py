...
# Training Parameters
learning_rate = 0.1 #change for Adam(0.001) and ADA(0.1) optimizer
batch_size = 100
hm_epochs = 15
# Network Parameters
width = 35
...
datafile = 'J_All_PM5_Image_Files'
print ("\nData file : ", datafile)
...
logs_dir = os.path.join(root_dir, 'K_TensorBoard')
...
train = pd.read_csv(os.path.join(sub_dir, 'all_image.csv'))
test = pd.read_csv(os.path.join(sub_dir, 'test_image.csv'))
sample_submission = pd.read_csv(os.path.join(sub_dir, 'Sample_Submission.csv'))

train.head()

print("\nSize of Training Set : ", len(train))
...
def conv_net(inputs, dropout):
    ...
    with tf.variable_scope('Single Convolutional Neural Network'):
        x = inputs
...
def train_neural_network(x):
    ...
    with tf.Session() as sess:
        ...
        sample_submission.to_csv(os.path.join(sub_dir, 'sub_1cnn_post.csv'), index=False)

train_neural_network(x)

sample_dir = os.path.join(sub_dir, 'sub_1cnn_post.csv')
